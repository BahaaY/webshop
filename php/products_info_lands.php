    ...
    if (isset($_POST['btn_sell'])) {
        ...
        if(!empty(array_filter($_FILES['input_image']['name']))) {
            if(count(array_filter($_FILES['input_image']['name'])) < 8){
                if ($price_lb_i==1 && $price_usd_i==1  && $title_i==1 && $description_i==1 && $type_i==1 && $size_i==1 && $location_i==1) {
                    $insert=$item->sell_product("product_land");
                    if($insert == 1){
                        ...
                        $item->insert_image_product("product_land");
